...
def ensemble_member_blockbootstrap(l):
    ...
    mean_con,sigma_con,weights_con,gm=GMM_timedep((salt_list[real][0*12:5*12,:,:].mean('time')).where(salt_list[real][0,:,:].latitude<65),n,'2011 to 2014') #take mean over first year

    from clustering_tools import clusters
    y,a2=clusters(gm,salt_list[real][0*12:5*12,:,:].mean('time'),'Location of each Gaussian, categorized by years 2011 to 2014 (mean), CESM',n)
    ...
    n=6
    from linear_response_tools import linear_response_list_bootstrap_lineartrend
    ...
    change_water1, individual_variance_water1, change_heat1, individual_variance_heat1=linear_response_list_bootstrap_lineartrend(salt_list_bootstrap,temp_list_bootstrap,salt_list[real][0:45*12,:,:],n,a2,weighted=1,area_cluster=area_cluster)
    
    std_including_individual_variances=np.sqrt((individual_variance_water1+change_water1**2).mean()-(change_water1.mean())**2)
    std=change_water1.std()
    mean=change_water1.mean()
    return std,std_including_individual_variances,mean,change_water1,individual_variance_water1
...
std_including_individual_variances2=np.empty(34)
change_water=np.empty([34,3000])
individual_variance_water=np.empty([34,3000])

#iterate the above function over the 34 members
for i in range(0,34): 
    std_member2[i],std_including_individual_variances2[i],mean_member2[i],change_water[i,:],individual_variance_water[i,:]=ensemble_member_blockbootstrap(i)
    print(i)


#pickle the results
with open("bootstrap_std_3000_2011to2055_areaweight_lintrend_12sept", "wb") as fp:   #Pickling
    pickle.dump(std_member2, fp)
with open("bootstrap_std_including_indiv_variance_3000_2011to2055_areaweight_lintrend_12sept", "wb") as fp:   #Pickling
    pickle.dump(std_including_individual_variances2, fp)
with open("bootstrap_mean_3000_2011to2055_areaweight_lintrend_12sept", "wb") as fp:   #Pickling
    pickle.dump(mean_member2, fp)
with open("bootstrap_change_water_3000_2011to2055_areaweight_lintrend_12sept","wb") as fp:
    pickle.dump(change_water, fp)
with open("bootstrap_individual_variance_water_3000_2011to2055_areaweight_lintrend_12sept","wb") as fp:
    pickle.dump(individual_variance_water, fp)
