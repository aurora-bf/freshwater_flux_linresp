...
def ensemble_member_blockbootstrap(l):
    ...
    change_water1, change_heat1,water_boot_modelspread,heat_boot_modelspread=linear_response_list_bootstrap(salt_list_bootstrap,temp_list_bootstrap,salt_list[real][0:45*12,:,:],n,a2,weighted=1,area_cluster=area_cluster,returns=4)
    
    std=water_boot_modelspread.std()
    mean=water_boot_modelspread.mean()
    return std,mean,water_boot_modelspread
...
#pickle the results
with open("bootstrap_std_9000_2011to2050_weighted_modelspread", "wb") as fp:   #Pickling
    pickle.dump(std_member2, fp)
with open("bootstrap_mean_9000_2011to2050_weighted_modelspread", "wb") as fp:   #Pickling
    pickle.dump(mean_member2, fp)
with open("bootstrap_change_water_9000_2011to2050_weighted_modelspread","wb") as fp:
