...
#pickle the results
with open("pvalue_linear_trends", "wb") as fp:   #Pickling the pvalues for each region for each ensemble member from the linear trend
    pickle.dump(pvalue_store, fp)
with open("pvalue_from_bootstrap", "wb") as fp:   #Pickling the pvalues for each region for each ensemble member from block bootstrapping
