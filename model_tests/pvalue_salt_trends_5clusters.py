...
pvalue_store=np.empty([34,5]) #p value from original linear fit
pvalue_bootstrap_store=np.empty([34,5]) #p value from block bootstrapped confidence interval
...
for real in range(0,34):
    ...
    bootstrapped_slope=np.empty([B,5])
    for i in range(0,B):
        for j in range(0,5):
...
#pickle the results
with open("pvalue_linear_trends_5clusters", "wb") as fp:   #Pickling the pvalues for each region for each ensemble member from the linear trend
    pickle.dump(pvalue_store, fp)
with open("pvalue_from_bootstrap_5clusters", "wb") as fp:   #Pickling the pvalues for each region for each ensemble member from block bootstrapping
