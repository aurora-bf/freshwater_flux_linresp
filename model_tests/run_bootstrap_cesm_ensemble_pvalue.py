# This file iterates through each CESM ensemble member and for each applies the method by first fitting a GMM, then creating an artificial ensemble using block bootstrapping, then applying linear response theory. It also finds the pvalues associated with the salinity trends both by first finding the and associated p value from the linear regression and by inverting the confidence interval after block bootstrapping (to find p value)
#Note for comparison with older versions of this code: This file basically combines pvalue_salt_trends.py and run_bootstrap_cesm_ensemble.py and makes the GMM happen on 3 year average instead of 5 (1975 to 1978) so it is consistent with the obs and cesm ensemble mean work
...
def ensemble_member_blockbootstrap(l):
    ...
    mean_con,sigma_con,weights_con,gm=GMM_timedep((salt_list[real][0*12:3*12,:,:].mean('time')).where(salt_list[real][0,:,:].latitude<65),n,'1975 to 1980 Historical') #take mean over first year

    from clustering_tools import clusters
    y,a2=clusters(gm,salt_list[real][0*12:3*12,:,:].mean('time'),'Location of each Gaussian, categorized by years 1975-1980 (mean), CESM',n)
    ...
    pvalue_bootstrap=np.empty([6])
    for j in range(0,6):
        z=bootstrapped_slope[:,j].mean()/bootstrapped_slope[:,j].std()
        pvalue_bootstrap[j] = scipy.stats.norm.sf(abs(z))
    ...
    return std,mean,change_water1,pvalue,pvalue_bootstrap
...
for i in range(0,34): 
    std_member2[i],mean_member2[i],change_water[i,:],pvalue_store[i,:],pvalue_bootstrap_store[i,:]=ensemble_member_blockbootstrap(i)
    print(i)

#pickle the results
with open("bootstrap_std_3000_areaweight_update", "wb") as fp:   #Pickling
    pickle.dump(std_member2, fp)
with open("bootstrap_mean_3000_areaweight_update", "wb") as fp:   #Pickling
    pickle.dump(mean_member2, fp)
with open("bootstrap_change_water_3000_areaweight_update","wb") as fp:
    pickle.dump(change_water, fp)
with open("pvalue_linear_trends_update", "wb") as fp:   #Pickling the pvalues for each region for each ensemble member from the linear trend
    pickle.dump(pvalue_store, fp)
with open("pvalue_from_bootstrap_update", "wb") as fp:   #Pickling the pvalues for each region for each ensemble member from block bootstrapping
