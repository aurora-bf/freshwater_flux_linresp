...
#The first function applies to a single timeseries of clustered salt and temperature. The second function takes a list of salt and temp fields and reclusters for each one and applies lin resp theory, and the third funciton takes a list of salt and temp fields that are bootstrapped (already clustered, not in lat and lon coordinates)
###############
...
def linear_response_single(salt,temp,a2,salt_mean,n,weighted=0,area_cluster=np.ones(6),title='Freshwater flux forcing over period 1975 to 2019 (45 years) compared to FAFMIP for IAP observations',output_everything=0):
   #----------------------------------------------------
    ...
    #This function has input of:
        #- salt: Surface salt that we are applying linear response theory to. Each list member input in time,cluster format. input over the period of time that you want analyzed
        # - temp: Surface temp that we are applying linear response theory to. Each list member input in time,cluster format. input with time cut to period of time that you want analyzedthat you want analyzed
    ...
    # This function has output of:
        # - change_water:Proportion of FAFMIP freshwater flux over period of interest. Calculated as mean of last 5 years minus first 5 years
        # - change_heat: Proportion of FAFMIP heat flux over period of interest. Calculated as mean of last 5 years minus first 5 years
        #output_everything (added as of 7 sept 2023) if turned to 1 also outputs df3 (the timeseries of f^w(t) and f^h(t)). by default it is off
    #-----------------------------------------------------
    #Load in response functions (from FAFMIP)
    ...
    for p in range(0,3):
        ...
        for start in range(start_yr,end_yr):
            ...
            a=np.linalg.norm(temp)/np.linalg.norm(salt)
            ...
            change_iap2_stack=np.concatenate((salt,temp/a),axis=1)
    ...
    change_water=np.mean(df3_mean[40:45,0])-np.mean(df3_mean[0:5,0])
    change_heat=np.mean(df3_mean[40:45,1])-np.mean(df3_mean[0:5,1])

    plt.plot(np.linspace(1975,2019,45),df3_mean[0:45,0],linewidth=3, color = (0,0.447,0.698))
    plt.plot(np.linspace(1975,2019,45),df3[0:45,0,0],':',linewidth=2,color = (0.902,0.624,0))
    plt.plot(np.linspace(1975,2019,45),df3[0:45,0,1],':',linewidth=2,color = (0,0.620,0.451))
    plt.plot(np.linspace(1975,2019,45),df3[0:45,0,2],':',linewidth=2,color = (0.8,0.475,0.655))
    ...
    if output_everything==0:
        return change_water,change_heat
    elif output_everything==1:
        return change_water,change_heat,df3_mean
...
#input a list of salt and temp
    ...
    for q in range(0,len(salt)):
        ...
        change_cesm=salt_cesm2[:,:,q]-np.mean(salt_cesm2[0:2,:,q],0)
        change_temp_cesm=temp_cesm2[:,:,q]-np.mean(temp_cesm2[0:2,:,q],0)
...
def linear_response_list_bootstrap(salt,temp,salt_mean,n,a2,obs=0,weighted=0,area_cluster=np.ones(6),returns=2): 
   #----------------------------------------------------
    #This function takes in salt and temperature fields as a bootstrapped list. Thus, the preprocessing of clustering and then bootstrapping around has already been done
    #This function has input of:
        # - salt: List of the surface salt fields that we are applying linear response theory to. Each list member input in time,cluster format. input over the period of time that you want analyzed
        # - temp: List of the surface temp fields that we are applying linear response theory to. Each list member input in time,cluster format. input with time cut to period of time that you want analyzed
        # - a2: the index locations in the vector of salinities x=np.linspace(31,38,10000) where each Gaussian starts and stops from clustering of the realization
        # - salt_mean: give it the salt field that was clustered on so that it can put the fafmip data in the correct clusters (time,latitude,longitude format)
        # - n: number of clusters
...
def linear_response_list_bootstrap_lineartrend(salt,temp,salt_mean,n,a2,obs=0,weighted=0,area_cluster=np.ones(6)): 
   #----------------------------------------------------
    #This is exactly the same as linear_response_list_bootstrap except it was made 12 september 2023 to deal with reviewer comments. Rather than take the last 5 years minus the first 5 years, we will fit a new linear trend to each block bootstrapped time series that had linear response theory applied to it
...
#input a list of salt and temp
    ...
    change_heat_1=np.empty([len(salt)])
    individual_variance_water_1=np.empty([len(salt)])
    individual_variance_heat_1=np.empty([len(salt)])
    change_water_modelspread=np.empty([len(salt),3])
    change_heat_modelspread=np.empty([len(salt),3])
    ...
    for q in range(0,len(salt)):
        ...
        p_water=scipy.stats.linregress(np.linspace(0,44,45), y=df3_mean[:,0], alternative='two-sided')
        p_heat=scipy.stats.linregress(np.linspace(0,44,45), y=df3_mean[:,1], alternative='two-sided')


        change_water_1[q]=(p_water.slope*np.linspace(0,44,45))[-1]
        change_heat_1[q]=(p_heat.slope*np.linspace(0,44,45))[-1]
        individual_variance_water_1[q]=(p_water.stderr*45)**2
        individual_variance_heat_1[q]=(p_heat.stderr*45)**2
        
    return change_water_1, individual_variance_water_1, change_heat_1, individual_variance_heat_1